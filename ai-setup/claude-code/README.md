...
read and write real state — relayed to the desktop app on that Mini, which is what
starts them. They are reachable only from machines on my Tailscale network, so this is
the one layer a cloud session does not get — see
